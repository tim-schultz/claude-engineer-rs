...
impl ToolExecutor {
    ...
    pub async fn apply_edits(
        ...
        let mut edited_lines: Vec<String> = original_content.lines().map(String::from).collect();
        ...
        for (i, edit) in edit_instructions.iter().enumerate() {
            ...
            let replace_lines: Vec<String> = edit
                .replace
                .lines()
                .map(|l| self.normalize_whitespace(l))
                .collect();
            ...
            'outer: for start_index in 0..edited_lines.len() {
                ...
                if match_found {
                    ...
                    edited_lines.splice(start_index..=end_index, replace_lines.iter().cloned());
                    changes_made = true;
                    edit_applied = true;
                    break 'outer;
                }
